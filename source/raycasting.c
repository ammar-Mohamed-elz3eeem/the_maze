...
void raycasting(int *maze, int textured)
{
	...
	Ver2F_t rayPos; /* X/Y coordinates of ray pos */
	Ver2F_t rayDir; /* direction of X/Y coordinates of ray pos */
	Ver2F_t posToNext; /* length of ray from current pos to next X/Y side */
	Ver2F_t distToNext; /* length of ray from X/Y side to next X/Y side */
	...
	for (x = 0; x < SCREEN_WIDTH; x++)
	{
		...
		render_walls(maze, map, rayPos, rayDir, distToWall, x, side, textured);
	}
